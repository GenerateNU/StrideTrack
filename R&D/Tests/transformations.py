...
def main():
    ...
    data_dir = PROJECT_ROOT / "R&D" / "Tests" / "Two_finger" / "Raw_data"
    data_dir.mkdir(parents=True, exist_ok=True)

    unified_files = sorted(data_dir.glob("*_BothFeet.csv"))
    ...
    if not unified_files:
        print(f"No *_BothFeet.csv files found in: {data_dir}")
        return
    ...
    for input_file in unified_files:
        ...
        out_df = transform_feet_to_stride_cycles(
            input_file,
            time_col="Time",
            foot1_col="Force_Foot1",
            foot2_col="Force_Foot2",
            foot1_label="left",
            foot2_label="right",
            force_threshold=0,
            dropout_fill_ms=20
