...
export const trainingRunCreateSchema = z.object({
  athlete_name: z.string().min(1, 'Athlete name is required').max(255, 'Athlete name must be 255 characters or less'),
  distance_meters: z.number().int('Distance must be an integer').positive('Distance must be greater than 0'),
  duration_seconds: z.number().positive('Duration must be greater than 0'),
  avg_ground_contact_time_ms: z
    .union([z.number().positive('Ground contact time must be greater than 0'), z.null()])
    .optional(),
});
...
export const trainingRunUpdateSchema = z.object({
  athlete_name: z.string().min(1, 'Athlete name must be at least 1 character').max(255, 'Athlete name must be 255 characters or less').nullable().optional(),
  distance_meters: z.number().int('Distance must be an integer').positive('Distance must be greater than 0').nullable().optional(),
  duration_seconds: z.number().positive('Duration must be greater than 0').nullable().optional(),
  avg_ground_contact_time_ms: z
    .union([z.number().positive('Ground contact time must be greater than 0'), z.null()])
    .optional(),
});
