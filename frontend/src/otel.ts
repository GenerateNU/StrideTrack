...
if (endpoint?.trim()) {
  const url = endpoint.trim().replace(/\/$/, "") + (endpoint.includes("/v1/traces") ? "" : "/v1/traces");

  const { BatchSpanProcessor, WebTracerProvider } = await import("@opentelemetry/sdk-trace-web");
