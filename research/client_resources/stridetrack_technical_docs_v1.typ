...
*FastAPI:* Async support and automatic OpenAPI documentation. Dependency injection via
`Depends()` enables clean separation between auth, services, and routes.

*Supabase:* Managed PostgreSQL with built-in Auth, row-level security, and a Python client.
Eliminates the need for self-hosted database and user management infrastructure.

*Pandas + NumPy:* Efficient vectorized processing of raw sensor CSV data. Handles
dropout fill and stride extraction without row-by-row Python loops.

*TanStack Query:* Declarative server state management with automatic caching, background
refetching, and loading/error states. Eliminates boilerplate `useEffect` + `useState` patterns.

*Zod:* Runtime schema validation on API responses catches shape mismatches at the
boundary rather than silently passing bad data into the UI.

*shadcn/ui:* Unstyled, accessible component primitives composed with TailwindCSS.
Avoids opinionated library styling while maintaining consistency.

*OpenTelemetry + Jaeger:* Distributed trace context propagated through all requests.
Correlates logs, spans, and errors without vendor lock-in.
...
For each stance interval, pair it with the next IC of the same foot to compute:

#table(
  columns: (auto, 1fr),
  [Column], [Formula],
  [`gct_ms`], [`to_time − ic_time`],
  [`flight_ms`], [`next_ic_time − to_time`],
  [`step_time_ms`], [`gct_ms + flight_ms`],
)
...
No global client-state store (no Redux, Zustand, etc.) --- all shared state is either
server-sourced or derived from auth context.
...
export function useGetAllRuns() {
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["runs"],
    queryFn: () => runService.getAllRuns(),
  });
  return { runs: data, runsIsLoading: isLoading, runsError: error, runsRefetch: refetch };
}
```
...
hurdles_60m, hurdles_100m, hurdles_110m, hurdles_400m, hurdles_partial,
long_jump, triple_jump, high_jump, bosco_test
```

*`role_enum`:* `coach`, `athlete`
...
Uploads sensor CSV and creates a new run with metrics. Request: `multipart/form-data`.
...
#table(
  ...
  [`file`], [CSV file], [Yes], [],
  [`athlete_id`], [UUID], [Yes], [],
  [`event_type`], [event_type_enum], [Yes], [],
  [`name`], [string], [No], [],
  [`target_event`], [event_type_enum], [No], [For `hurdles_partial` only],
)
...
#table(
  ...
  [`athlete_id`], [UUID], [Yes], [Target athlete],
  [`event_type`], [event_type_enum], [Yes], [Filter by event],
