...
class TestCountStepsBetween:
    ...
    def test_counts_correctly(self, steps_df: pd.DataFrame) -> None:
        ...
        result = _count_steps_between(
            steps_df, t_start_exclusive=700, t_end_exclusive=1100
        )
        assert result == 1
    ...
    def test_excludes_boundary_values(self, steps_df: pd.DataFrame) -> None:
        ...
        result = _count_steps_between(
            steps_df, t_start_exclusive=700, t_end_exclusive=910
        )
        assert result == 0
    ...
    def test_empty_range_returns_zero(self, steps_df: pd.DataFrame) -> None:
        ...
        result = _count_steps_between(
            steps_df, t_start_exclusive=500, t_end_exclusive=500
        )
        assert result == 0
    ...
    def test_empty_df_returns_zero(self, empty_steps_df: pd.DataFrame) -> None:
        ...
        result = _count_steps_between(
            empty_steps_df, t_start_exclusive=0, t_end_exclusive=9999
        )
        assert result == 0
...
class TestTransformStrideCyclesToHurdleMetrics:
    ...
    def test_empty_input_returns_empty_with_schema(
        self, empty_steps_df: pd.DataFrame
    ) -> None:
