...
class TestFillShortZeroDropouts:
    ...
    def test_mixed_short_and_long_gaps(self) -> None:
        ...
        result = _fill_short_zero_dropouts_in_contact(contact, max_hole_len_samples=2)
        np.testing.assert_array_equal(result, [True, True, True, False, False, False, True])
...
class TestExtractStanceIntervals:
    ...
    def test_empty_arrays_return_empty_df(self) -> None:
        ...
        assert result.empty
        assert list(result.columns) == ["ic_idx", "to_idx", "ic_time_raw", "to_time_raw"]
...
class TestBuildStrideRows:
    ...
    def _make_stance_df(self, intervals: list[tuple[int, int, int, int]]) -> pd.DataFrame:
        """Create a stance df from (ic_idx, to_idx, ic_time_raw, to_time_raw) tuples."""
        return pd.DataFrame(intervals, columns=["ic_idx", "to_idx", "ic_time_raw", "to_time_raw"])
    ...
    def test_empty_stance_df_returns_empty(self) -> None:
        ...
        stance_df = pd.DataFrame(columns=["ic_idx", "to_idx", "ic_time_raw", "to_time_raw"])
 
        result = _build_stride_rows(stance_df, "left", t0_raw=0)
 
        assert result.empty
        expected_cols = ["foot", "ic_time", "to_time", "next_ic_time", "gct_ms", "flight_ms", "step_time_ms"]
...
class TestTransformFeetToStrideCycles:
    ...
    def test_empty_dataframe_returns_empty_with_correct_columns(self) -> None:
        ...
        assert result.empty
        expected_cols = [
            "stride_num", "foot", "ic_time", "to_time",
            "next_ic_time", "gct_ms", "flight_ms", "step_time_ms",
        ]
        assert list(result.columns) == expected_cols
