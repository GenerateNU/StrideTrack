...
class HurdleService:
    ...
    async def _get_hurdle_metric_rows(self, run_id: UUID, hurdles_completed: int | None = None) -> list[HurdleMetricRow]:
    ...
    async def get_hurdle_metrics_by_run_id(self, run_id: UUID, hurdles_completed: int | None = None) -> list[HurdleMetricRow]:
        """Get all per-hurdle metrics for a run."""
        logger.info(f"Service: Getting hurdle metrics for run {run_id}")
        result = await self._get_hurdle_metric_rows(run_id, hurdles_completed=hurdles_completed)
        logger.info(f"Service: Returning {len(result)} hurdle rows for run {run_id}")
        return result

    async def get_hurdle_splits(self, run_id: UUID, hurdles_completed: int | None = None) -> list[HurdleSplitBarData]:
        """Transform hurdle data for split bar chart."""
        logger.info(f"Service: Getting hurdle splits for run {run_id}")
        data = await self._get_hurdle_metric_rows(run_id, hurdles_completed=hurdles_completed)
    ...
    async def get_takeoff_gct(self, run_id: UUID, hurdles_completed: int | None = None) -> list[TakeoffGctBarData]:
    ...
    async def get_landing_gct(self, run_id: UUID, hurdles_completed: int | None = None) -> list[LandingGctBarData]:
    ...
    async def get_takeoff_ft(self, run_id: UUID, hurdles_completed: int | None = None) -> list[TakeoffFtBarData]:
    ...
    async def get_gct_increase(self, run_id: UUID, hurdles_completed: int | None = None) -> list[GctIncreaseData]:
    ...
    async def get_hurdle_projection(self, run_id: UUID, hurdles_completed: int | None = None) -> HurdleProjectionResponse:
        ...
        completed_metrics = await self._get_hurdle_metric_rows(run_id, hurdles_completed=hurdles_completed)
        result = project_hurdle_race(completed_metrics, target_event)
        return HurdleProjectionResponse(**result)

    async def get_hurdle_timeline(self, run_id: UUID, hurdles_completed: int | None = None) -> HurdleTimelineResponse:
        """Build time-series points for the hurdle timeline chart."""
        logger.info(f"Service: Getting hurdle timeline for run {run_id}")
        steps = await self._get_hurdle_metric_rows(run_id, hurdles_completed=hurdles_completed)
